...
WORKDIR /src
COPY ["Momentum.Pixel.Lambda.Api.csproj", "Momentum.Pixel.Lambda.Api/"]
RUN dotnet restore "Momentum.Pixel.Lambda.Api/Momentum.Pixel.Lambda.Api.csproj"

WORKDIR "/src/Momentum.Pixel.Lambda.Api"
COPY . .
RUN dotnet build "Momentum.Pixel.Lambda.Api.csproj" --configuration Release --output /app/build

FROM build AS publish
RUN dotnet publish "Momentum.Pixel.Lambda.Api.csproj" \
...
Execute unit tests
```
    cd "Momentum.Pixel.Lambda.Api/test/Momentum.Pixel.Lambda.Api.Tests"
    dotnet test
```

Deploy application
```
    cd "Momentum.Pixel.Lambda.Api/src/Momentum.Pixel.Lambda.Api"
